...
export class LearnlanguageComponent implements OnInit, OnDestroy {
  ...
  async handleSpeechResponse() {
    ...
    this.computerSpeech(response); // Respond vocally when using speech input
  }
  ...
  async generateDynamicResponse(input: string): Promise<string> {
    try {
      const response = await this.geminiService.text(input);
