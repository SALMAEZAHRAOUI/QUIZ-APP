...
export class GeminiService {
  ...
  constructor() {
    ...
    this.generativeAI = new GoogleGenerativeAI('AIzaSyCIvMQfkUe7HBK4VjzwM-WL1FGsf0UfHDY');
  }
  async generateText2(question: string): Promise<string> {
    try {
      // Initialize the generative model
      const model = this.generativeAI.getGenerativeModel({ model: 'gemini-pro' });
  
      const result = await model.generateContent("Please provide an insightful and helpful response less than 20 words to the following question in a small paragraph and  don't use special characters like '*+-' : ${question}");
...
async text(prompt: string): Promise<string> {
  try {
    const model = this.generativeAI.getGenerativeModel({ model: 'gemini-pro' });
    // Updated prompt to allow full-text responses
    const result = await model.generateContent(`Provide a detailed response in : ${prompt}`);
    const response = await result.response.text();
    console.log('Gemini response:', response);
    return response.trim(); // Trim any extra spaces from the response
  } catch (error) {
