...
export class LogComponent {
  ...
  onSubmit() {
    ...
    if (errorMessage) {
      ...
      console.log('Form submitted successfully:', { email: this.username, password: this.password });
  ...
  navigateToabout(){
   ...
   async login() {
    try {
      ...
      this.token = res.token; // Stocke le token JWT
      alert('Connexion réussie !');
      this.router.navigate(['/secret']); // Redirige vers une autre page (si nécessaire)
    } catch (err: any) {
